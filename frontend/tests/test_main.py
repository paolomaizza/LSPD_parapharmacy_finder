    ...
    with patch('app.main.fetch_regions_from_backend', return_value=[
        ...
        with patch('requests.get') as mock_get:
            ...
            mock_get.return_value.json.return_value = {
                'parapharmacies': [
                    {"name": "Parapharmacy1", "address": "Address1",
                     "cap": "1001", "city": "City1"},
                    {"name": "Parapharmacy2", "address": "Address2",
                     "cap": "1002", "city": "City2"}
                ]
            }
...
def test_internal_post_invalid_region(client):
    with patch('app.main.fetch_regions_from_backend', return_value=[
        ...
        with patch('requests.get') as mock_get:
            ...
            assert response.status_code == 200
            soup = BeautifulSoup(response.data, 'html.parser')
            assert "Invalid region code 999" in soup.text, \
                f"Response data: {soup.prettify()}"
...
def test_internal_post_no_parapharmacies(client):
    with patch('app.main.fetch_regions_from_backend', return_value=[
        ...
        with patch('requests.get') as mock_get:
            ...
            assert response.status_code == 200
            soup = BeautifulSoup(response.data, 'html.parser')
            assert "No parapharmacies available in region 1" in soup.text, \
                f"Response data: {soup.prettify()}"
